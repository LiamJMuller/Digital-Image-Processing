...
min_val=min(oImg(:));
max_val=max(oImg(:));
enhImg2 = imadjust(oImg, [min_val max_val], [0 1]);
...
% sepia tones
enhColour(:,:,1) = enhImg*1.0; % Red channel
enhColour(:,:,2) = enhImg*0.9; % Green channel
enhColour(:,:,3) = enhImg*0.7; % Blue channel

enhColour=min(enhColour,1); % ensure no values above 255
enhColour=max(enhColour,0);
